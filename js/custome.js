...
class ScrollStyle{
    ...
    */ 
    ...
    ## 
    */ 
    changeActiveWhenDown = (sections = 'section', linkes = '.nav-link', active = 'active') =>{
    ...
    toggleSidebar = (duration=1500)=>{
        $(this.toggleBtn).click(()=>{
            ...
            if($(this.sidebar).css('left') == '0px'){
                $(this.sidebar).animate({'left':`-${this.sidebarWidth}px`}, duration)
            }else{
                $(this.sidebar).animate({'left': 0}, duration)
            }
        })
    }
    /*
...
function switchNavActiveLink(links='.nav-link', active='active', duration=1000){
    $(links).click(function(){
        ...
        $('body,html').animate({scrollTop: secTop}, duration)
    })
}
